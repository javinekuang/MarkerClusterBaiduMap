...
import k.javine.markerclusterbaidumap.R;
...
public class DefaultClusterRenderer<T extends ClusterItem> implements
            ...
            for (Cluster<T> c : clusters) {
                ...
                if (zoomingIn && onScreen && SHOULD_ANIMATE) {
                    ...
                    if (closest != null) {
                        ...
                        markerModifier.add(true, new CreateMarkerTask(c, newMarkers, null));//什么情况下会执行这一句？
                    }
                } else {
    ...
    private class MarkerModifier extends Handler implements MessageQueue.IdleHandler {
        ...
        private Queue<CreateMarkerTask> mCreateMarkerTasks = new LinkedList<CreateMarkerTask>();//屏幕外新增marker任务队列
        private Queue<CreateMarkerTask> mOnScreenCreateMarkerTasks = new LinkedList<CreateMarkerTask>();//屏幕内新增marker任务队列
        private Queue<Marker> mRemoveMarkerTasks = new LinkedList<Marker>();//屏幕外删除marker队列
        private Queue<Marker> mOnScreenRemoveMarkerTasks = new LinkedList<Marker>();//屏幕内删除marker队列
        private Queue<AnimationTask> mAnimationTasks = new LinkedList<AnimationTask>();//动画任务队列
